				...
				<th colspan="4" align="right">
					<div style="display: inline-block;">
						<form action="${pageContext.request.contextPath }/SearchProductServlet" method="post">
							<select name="cid">
								...
								<c:if test="${not empty categories }">
									<c:forEach items="${categories}" var="category">
										<c:choose>
											<c:when test="${!empty pr.cid && category.cid == pr.cid }">
							...
							<input type="text" name="pname" value="${pr.pname }" />
			...
			<tr>
				<td colspan="7" align="center">
					<c:choose>
						...
						<c:otherwise>
							[ <a href="${pageContext.request.contextPath }/SearchProductServlet?pageNumber=1&cid=${pr.cid }&pname=${pr.pname }">首页</a> ]
							[ <a href="${pageContext.request.contextPath }/SearchProductServlet?pageNumber=${pageBean.pageNumber - 1 }&cid=${pr.cid }&pname=${pr.pname }">上一页</a> ]
						</c:otherwise>
					</c:choose>
					<c:forEach begin="1" end="${pageBean.totalPageNumber }" var="i" step="1">
						<c:choose>
							...
							<c:when test="${i > pageBean.pageNumber - 3 && i < pageBean.pageNumber + 3}">
								[ <a href="${pageContext.request.contextPath }/SearchProductServlet?pageNumber=${i }&cid=${pr.cid }&pname=${pr.pname }">${i }</a> ] 
							</c:when>
						</c:choose>
					</c:forEach>
					...
					<c:choose>
						...
						<c:otherwise>
							[ <a href="${pageContext.request.contextPath }/SearchProductServlet?pageNumber=${pageBean.pageNumber + 1 }&cid=${pr.cid }&pname=${pr.pname }">下一页</a> ]
							[ <a href="${pageContext.request.contextPath }/SearchProductServlet?pageNumber=${pageBean.totalPageNumber }&cid=${pr.cid }&pname=${pr.pname }">尾页</a> ]
						</c:otherwise>
					</c:choose>
